...
import {
  ...
  Building2, FileQuestion, CheckSquare, BarChart3, FileText, Menu, X, ChevronRight, Home,
} from 'lucide-react';
...
export default function ErpApp({ onHome }: ErpAppProps) {
      ...
      <div className="min-h-screen bg-slate-50">
        <header className="lg:hidden bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-50 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-[#0F4C81] to-blue-700 rounded-xl flex items-center justify-center">
          ...
          <div className="lg:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setMobileMenuOpen(false)}>
            <div className="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-xl overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <nav className="p-3 space-y-1">
                {onHome && (
                  <button onClick={() => { onHome(); setMobileMenuOpen(false); }}
                    className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-slate-600 hover:bg-slate-50">
                    <Home className="w-5 h-5" /><span className="font-medium">Back to Website</span>
                  </button>
                )}
                <button onClick={() => { setActiveModule(null); setMobileMenuOpen(false); }}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${!activeModule ? 'bg-[#0F4C81] text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
        ...
        <aside className={`hidden lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:flex-col bg-white border-r border-slate-200 transition-all duration-300 z-30 ${sidebarOpen ? 'lg:w-64' : 'lg:w-[68px]'}`}>
          <div className="h-16 flex items-center px-4 border-b border-slate-200">
            <button type="button" onClick={() => onHome?.()} className="flex items-center gap-3 overflow-hidden hover:opacity-80 transition-opacity" title="Back to website">
              <div className="w-9 h-9 bg-gradient-to-br from-[#0F4C81] to-blue-700 rounded-xl flex items-center justify-center flex-shrink-0">
                <GraduationCap className="w-5 h-5 text-white" />
              </div>
          ...
          <nav className="flex-1 p-3 overflow-y-auto space-y-1">
            {onHome && (
              <button onClick={onHome}
                className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-slate-500 hover:bg-slate-50 transition-all text-sm">
                <Home className="w-5 h-5 flex-shrink-0" />
                <span className={`font-medium whitespace-nowrap transition-opacity duration-200 ${sidebarOpen ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>Website</span>
              </button>
            )}
            <button onClick={() => setActiveModule(null)}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-lg transition-all ${!activeModule ? 'bg-gradient-to-r from-[#0F4C81] to-blue-700 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}>
            ...
            {seeded ? <ActiveComponent /> : (
              <div className="flex items-center justify-center min-h-[60vh]">
                <div className="text-center text-slate-400">
                  <div className="w-10 h-10 border-4 border-[#0F4C81] border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
